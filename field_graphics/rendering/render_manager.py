...
def modelFromJSON(data: str):
    ...
    for obj in objects:
        ...
        for uniform in uniforms:
            ...
            if uniform_type == "int": GL.glUniform1i(loc, int(uniform["v0"]))
            ...
            elif uniform_type == "mat4": GL.glUniformMatrix4fv(loc, float(uniform["v0"]), float(uniform["v1"]), float(uniform["v2"]), float(uniform["v3"]),
                                                                    ...
                                                                    float(uniform["v12"]), float(uniform["v13"]), float(uniform["v14"]), float(uniform["v15"]))
        models.append(
